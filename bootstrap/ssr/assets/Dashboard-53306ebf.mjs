...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div style="${ssrRenderStyle({ "overflow": "scroll" })}" class="w-full h-[calc(100vh-50px)] mb-12 overflow-y-auto pt-4 px-4 pb-6 bg-gray-200 flex-1 flex flex-col">`);
      ...
      if (unref(display)) {
        _push(`<div class="w-full"><div class="w-full mt-8 bg-white flex flex-col justify-evenly items-center"><div class="w-full pl-2 text-sm leading-10 border-b-2 text-base">客户数据</div><div class="w-full h-20 border-b-2 pl-8 text-sm text-gray-400 flex flex-col justify-evenly"><a${ssrRenderAttr("href", "tel:" + tel.phone)}>客户手机号：${ssrInterpolate(tel.phone)}</a><div>获取数据时间：${ssrInterpolate(tel.created_at)}</div></div></div><div class="w-full px-6 py-3 border-b-2 bg-white flex flex-col items-center"><div class="w-full mt-1 pr-1 text-sm leading-8 border-b flex justify-between"><div class="text-gray-400">已接通</div>`);
        if (tel.status === 1) {
